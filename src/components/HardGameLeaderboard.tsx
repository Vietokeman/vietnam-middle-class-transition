...
import { getTopHardGameScores, getRecentHardGameScores, HardGameScore } from '@/services/hardGameService';

interface HardGameLeaderboardProps {
    isOpen: boolean;
    onClose: () => void;
    currentScore?: number;
}
...
const HardGameLeaderboard: React.FC<HardGameLeaderboardProps> = ({ isOpen, onClose, currentScore }) => {
    const [activeTab, setActiveTab] = useState<TabType>('top');
    const [topScores, setTopScores] = useState<HardGameScore[]>([]);
    const [recentScores, setRecentScores] = useState<HardGameScore[]>([]);
    ...
    const loadScores = async () => {
        ...
        try {
            const [top, recent] = await Promise.all([
                getTopHardGameScores(10),
                getRecentHardGameScores(10),
            ]);
                    ...
                    >
                        ...
                        <div className="bg-gradient-to-r from-vietnam-red-600 to-vietnam-gold-500 p-6 text-white">
                            ...
                            <div className="flex items-center gap-3 mb-4">
                                <Trophy size={32} />
                                <h2 className="text-2xl md:text-3xl font-bold">Bảng Xếp Hạng - Vua Mật Mã</h2>
                            </div>
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-sm text-gray-500">
                                                        {formatDate(scoreData.timestamp)} • ⏱️ {formatTime(scoreData.timeSeconds)}
                                                    </p>
                                                </div>
...
export default HardGameLeaderboard;
